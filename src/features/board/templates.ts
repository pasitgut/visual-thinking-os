...
import type { TaskNode, TaskNodeData } from "@/types/task";
...
export interface BoardTemplate {
  ...
  description: string;
  icon: string;
  nodes: (Omit<TaskNode, "data"> & { data: TaskNodeData })[];
  edges: Edge[];
}
...
export const prepareTemplate = (template: BoardTemplate) => {
  ...
  template.nodes.forEach((_node) => {
    // idMap[node.id] = node.id === "root" ? "root" : uuidv4();
  });

  const nodes = template.nodes.map((_node) => ({
    // ...node,
    // id: idMap[node.id],
    // data: { ...node.data },
  }));
