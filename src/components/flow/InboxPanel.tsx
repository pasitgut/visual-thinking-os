...
export const InboxPanel = () => {
  ...
  return (
    <div className={cn(
      "fixed top-0 left-0 h-screen w-full sm:w-[380px] bg-background border-r z-[150] shadow-2xl transition-transform duration-300 ease-in-out transform flex flex-col",
      ...
      <div className="flex items-center justify-between p-4 border-b bg-primary/5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold text-sm uppercase tracking-widest">Inbox</h2>
            <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-tighter">Quick Thoughts</p>
          </div>
        </div>
        <Button 
          variant="ghost" 
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-3">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 px-6 text-center space-y-4">
            ...
            <div>
              <p className="text-sm font-medium text-muted-foreground">Your inbox is empty</p>
              <p className="text-xs text-muted-foreground/60 mt-1 italic">Use Alt+I to capture thoughts quickly from anywhere</p>
            </div>
          </div>
        ) : (
          items.map((item) => (
      ...
      <div className="p-4 border-t bg-muted/20">
        <div className="flex items-center justify-center gap-2 py-2 px-3 bg-background/50 rounded-lg border border-border/50">
          <kbd className="px-1.5 py-0.5 rounded bg-foreground/10 text-[10px] font-mono border border-foreground/10">ALT + I</kbd>
          <span className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">to quick capture</span>
        </div>
      </div>
    </div>
  );
